...
for subject_folder in */; do
    ...
    # For simplicity with the CSV file, rename each NIfTI file by removing the first character of each IMAGE_ID.
    # Example: I1234.nii.gz -> 1234.nii.gz

    for file in "$subject_folder"*.gz; do
        ...
        echo "Renaming the image $filename to $new_filename"
        mv "$file" "$subject_folder$new_filename"
