...
for subject_folder in "subjects/"*/; do
    ...
    for mri_file in "$subject_folder"*; do
        ...
        if [ -f "$new_path/skullstrip.nii.gz" ] && [ -f "$new_path/skullstrip_mask.nii.gz" ] && [ -f "$new_path/normalized.nii.gz" ]; then
        ...
        if [ ! -f "$new_path/corrected.nii.gz" ]; then
        ...
        antsRegistrationSyNQuick.sh -d 3 -m "$new_path/corrected.nii.gz" -o "$new_path/" -t "a" -f "../MNI152_T1_1mm.nii.gz" > /dev/null
        mv "$new_path/Warped.nii.gz" "$new_path/registered.nii.gz"

        if [ ! -f "$new_path/registered.nii.gz" ]; then
        ...
        if [ ! -f "$new_path/skullstrip.nii.gz" ] || [ ! -f "$new_path/skullstrip_mask.nii.gz" ]; then
        ...
        ws-normalize -mo t1 -o "$new_path/normalized.nii.gz" "$new_path/skullstrip.nii.gz"

        if [ ! -f "$new_path/normalized.nii.gz" ]; then
            echo "Intensity Normalization failed"
            exit 1
        fi

        # We are just interested in Skull Stripping and Intensity Normalization files. 
        # Remove intermediate files, keeping only the final preprocessed files.

        find "$new_path" -type f ! \( -name "skullstrip*" -o -name "normalized*" \) -exec rm {} +
